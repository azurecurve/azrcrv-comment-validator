...
class UpdateClient {
	...
	private function init() {
		...
		add_action( 'admin_print_footer_scripts', array( $this, 'print_admin_scripts' ) );

		// Filter the admin row links.
		add_filter( $this->config['type'] . '_row_meta', array( $this, 'filter_component_row_meta' ), 10, 2 );

		// Filter update data into the transient before saving.
		add_filter( 'pre_set_site_transient_update_' . $this->config['type'] . 's', array( $this, 'filter_component_update_transient' ) );

		// Filter install API results.
		add_filter( $this->config['type'] . 's_api_result', array( $this, 'filter_components_api_result' ), 10, 3 );

		// Filter after-install process.
		add_filter( 'upgrader_post_install', array( $this, 'filter_upgrader_post_install' ), 11, 3 );
			...
			foreach ( $components as $component => $data ) {
				...
				if ( isset( $data['id'], $data['new_version'], $data['package'] ) ) {
					...
					if ( $this->config['type'] === 'plugin' ) {
						...
						// Plugin images.
						$icons = $banners = $screenshots = array();
	...
	private function get_component_data( $action, $component = '' ) {
		...
		} elseif ( $action === 'theme_information' ) {

			// If querying a single theme, assign it to the post body.
			$body[ $this->config['type'] ] = $component;

		} elseif ( $action === 'query_plugins' ) {
		...
		if ( is_wp_error( $raw_response ) ) {
			return new \WP_Error( 'update_manager_http_error', 'HTTP error. ' . $raw_response->get_error_message(), array( 'original_error' => $raw_response ) );
		}
		...
		$this->component_data = is_array( $data ) ? $data : array();
	...
	public function get_latest_version_number() {
		...
		$response = wp_remote_get( 'https://api-v1.classicpress.net/upgrade/index.php', array( 'timeout' => 3 ) );
